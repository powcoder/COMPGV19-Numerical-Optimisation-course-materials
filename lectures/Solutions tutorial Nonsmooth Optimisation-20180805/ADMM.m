function [u, v, w, iter, stopValue, uvIterates, info] = ADMM(f, A , Atr, invLS, E, Etr, F, b, Proxy, u0, para)
% ADMM implements the alternating direction method of multipliers to solve 
%   min E(x) = 1/2 * || A u - f ||_2^2 + J(u)
...
%   f           - the sensor data f
%   invLS       - function handle to argmin || A u - f ||_2^2 + rho || w - v ||_2^2
...
%   Proxy       - proxmial operator as a function handle of x and alpha to solve 
...
%        'primalDualRes' - both primal and dual residuum are small enough 
%                          (see Section 3.3.1. in Boyd et al.)
...
while(~stopFL && iter < maxIter)
    ...
    if isa(invLS,'function_handle')
      ...
      [u,flag,relres,iter,resvec] = pcg(@(x) Atr(A(x)) + rho*x, Atr(f) + rho*(v-w), tolLS, maxItLS);
    end    
